...
def Laplacian_blend(image1, image2, blending_filter):
    ...
    for i in range(len(lp1)):
        ...
        filter_of_im1 = np.multiply(lp1[i], filter_gaussian[i])
        negative_of_im2 = np.multiply(lp2[i],negative_filter_gaussian[i]) 
        ls = cv2.add(filter_of_im1, ls)
        ls = cv2.add(negative_of_im2, ls)
        LS.append(ls)
