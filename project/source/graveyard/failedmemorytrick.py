...
def draw_head(npimg,Centers,col,bol,k=[0]):
    ...
    if (neck and nose):
        ...
        #head pointing left (left eye hidden)
            ...
            rx,ry = int(dx*k4*0.85),int(dx*k4*0.73)
            cv2.ellipse(npimg,(hx,hy),(rx,ry)  ,-angle-20,0,360,col,-1)
...
def draw_humans(npimg, humans_tmp,humans,shit,bol=1,k=[0]): #main function
    npimg *=0
    image_h, image_w = npimg.shape[:2]
    centers = {}
    shit2 = []
    tryvar = lambda Centers, varpos: Centers[varpos] if varpos in Centers.keys() else None
    iterator = 0
    col = [255,255,255]
    for human in humans_tmp:
        for human_old in humans:
            for ellipse in shit:
                if sum(ellipse) == 0:
                    continue
                x,y,r = ellipse
                if tryvar(human.body_parts,0) and tryvar(human_old.body_parts,0):
                    r2 = (x-(human_old.body_parts[0].x*image_w+0.5))**2 + (y-(human_old.body_parts[0].y*image_h+0.5))**2        
                    r1 = (x-(human.body_parts[0].x*image_w+0.5))**2 + (y-(human.body_parts[0].y*image_h+0.5))**2        
                    print r1,r2,r
                
                    if r > r1 and r > r2:    
                
                        parts_old = human_old.body_parts.keys()
                        parts_new = human.body_parts.keys()
                        for i in range(common.CocoPart.Background.value):
                            if (i not in parts_new)*(i in parts_old):
                                human.body_parts[i] = human_old.body_parts[i]
        # draw point
        Centers={}
        ...
        hx,hy,r,npimg = draw_head(npimg,Centers,col,bol,k) #draws head (with internal jaw-line)
        npimg = draw_limbs(npimg,Centers,col,parts) #draws arms and legs
        npimg = draw_torso(npimg,Centers,col,parts) #sigh
        shit2.append([hx,hy,r])
    return npimg, shit2


def draw_humans_initial(npimg, humans,bol=1,k=[0]): #main function
    npimg *=0
    image_h, image_w = npimg.shape[:2]
    centers = {}
    col = [255,255,255]
